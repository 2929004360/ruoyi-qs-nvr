package com.ruoyi.gb28181.utils;

import com.alibaba.fastjson2.JSONArray;
import com.alibaba.fastjson2.JSONObject;
import com.ruoyi.gb28181.common.MessageElement;
import com.ruoyi.gb28181.common.MessageElementForCatalog;
...
public class XmlUtil {
     ...
     * @param json
     */
    public static void node2Json(Element element, cn.hutool.json.JSONObject json) {
        // 如果是属性
        for (Object o : element.attributes()) {
            Attribute attr = (Attribute) o;
            if (!ObjectUtils.isEmpty(attr.getValue())) {
                json.set("@" + attr.getName(), attr.getValue());
            }
        }
        List<Element> chdEl = element.elements();
        if (chdEl.isEmpty() && !ObjectUtils.isEmpty(element.getText())) {// 如果没有子元素,只有一个值
            json.set(element.getName(), element.getText());
        }
        ...
        for (Element e : chdEl) {   // 有子元素
            if (!e.elements().isEmpty()) {  // 子元素也有子元素
                ...
                if (o != null) {
                    ...
                    json.set(e.getName(), jsona);
                } else {
                    if (!chdjson.isEmpty()) {
                        json.set(e.getName(), chdjson);
                    }
                }
            } else { // 子元素没有子元素
                for (Object o : element.attributes()) {
