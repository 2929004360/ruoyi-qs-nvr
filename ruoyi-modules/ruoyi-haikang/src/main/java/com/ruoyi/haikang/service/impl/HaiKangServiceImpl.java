...
public class HaiKangServiceImpl implements IHaiKangService {
     ...
     * 设备信息
     */
    public ConcurrentHashMap<Long, HCNetSDK.NET_DVR_DEVICEINFO_V40> map = new ConcurrentHashMap<Long, HCNetSDK.NET_DVR_DEVICEINFO_V40>();

    /**
     * 海康登录用户ID
     */
    public ConcurrentHashMap<Long, Integer> userIdMap = new ConcurrentHashMap<Long, Integer>();
     ...
     */
    public int loginDevice(Long id, String ip, short port, String user, String psw) {
        // 创建设备登录信息和设备信息对象
    ...
    public Integer getUserId(Long id) {
        ...
        Integer userId = userIdMap.get(id);
        if (ObjectUtil.isNull(userId)) {
//            HaikangDevice haikangDevice = haikangDeviceService.selectHaikangDeviceById(id);
//            return loginDevice(haikangDevice.getId(), haikangDevice.getIp(), haikangDevice.getPort(), haikangDevice.getUserName(), haikangDevice.getPassword());
            return 0;
        } else {
